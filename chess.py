...
def pre_over(fen):
    ...
    resutl = False
    count = 0
    for tmp in tmp_moves:
        tmpp_moves = generate_legal_moves(tmp)
        for tmpp in tmpp_moves:
            resutl = is_game_over(tmpp)
            if (resutl):
                count += 1
                break
        if (not resutl and (count > 0)):
            # print(count)
            return True, tmp
    return False, ""
...
def update_old_fen(boardRow, rowIndex):
    ...
    for pRow, piece in enumerate(boardRow):
        if piece.isdigit():
            ...
            if tempIndex == rowIndex:
                # check again
                if pRow + 1 < len(boardRow) and boardRow[pRow + 1].isdigit():
                    num = 0
    ...
    for pRow, piece in enumerate(boardRow):
        if piece.isdigit():
            if tempIndex + int(piece) >= rowIndex:
                ...
                res += (str(num1) if num1 != 0 else '') + insertPiece + \
                    (str(num2) if num2 != 0 else '')  # return at this
                res += boardRow[pRow+1:]
                return res
            else:
                tempIndex += int(piece)
        ...
        for new_pCol in range(pCol + 1, pCol + 3):
            ...
            if 0 <= new_pCol < 8:
                if meet_pieces(board[new_pCol], pRow):
                    ...
                    newboard[new_pCol] = update_board_with_fen(
                        board[new_pCol], pRow, piece)
...
# fen = "8/8/1p6/2p5/2P1bk2/p5K1/8/n3r3 b"
...
# print(pre_over("3k2n1/4pp2/3p3B/p2P3p/P7/1PNP4/R3PPPP/3QKBNR b"))
